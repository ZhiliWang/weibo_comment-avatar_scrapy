...
def fetch_data(query_val, page_id):
    ...
    for card in card_group:
        ...
        blog = {'mid': mblog['id'],  # 微博id
                ...
                'avatar_url': mblog['user']['avatar_hd']
                }
        mblogs.append(blog)
    return mblogs
...
def urllib_download(image_url, name, key):
    from urllib.request import urlretrieve
    urlretrieve(image_url, f'./image/{key}/{name}.jpg')


def fetch_pages(query_val, page_num):
    os.makedirs('./image/', exist_ok=True)
    os.makedirs('./results/', exist_ok=True)
    os.makedirs(f'./image/{query_val}', exist_ok=True)
    """抓取关键词多页的数据"""
    mblogs = []
    ...
    # 保存到 result.json 文件中
    fp = open('./results/result_{}.json'.format(query_val),
              'w', encoding='utf-8')
    json.dump(mblogs, fp, ensure_ascii=False, indent=4)
    print("已保存至 ./results/result_{}.json".format(query_val))
    # 保存头像
    print(f'开始下载{query_val}关键词评论下的头像')
    for blog in mblogs:
        urllib_download(blog['avatar_url'], blog['mid'], query_val)
